...
// //Api for delete the data using id.
...
//             db.collection("Datametadata").findOne(query, function (err, data) {
//                 if (data == null) {
//                     res.status(500).json({ success: false, message: 'data not exist' });
//                 }
//                 else {
//                     console.log('data****', data);

//                     db.collection("Datametadata").remove(query, function (err, data) {
//                         if (err) {
//                             res.status(500).json({ success: false, message: 'something went wrong.' });
//                         }
//                         else {
//                             db.collection("datainserts").remove({ _id: ObjectID(req.params.id) }, function (err, data1) {
//                                 if (err) res.status(500).json({ success: false, message: 'something went wrong.' })
//                                 else {
//                                     console.log('data1', data1)
...
//    if(data == null)
//    {     
//        res.status(404).json({success:false, message : 'data not fount' })
//    }
//    else{
//        db.collection("datainserts").remove(query)
//    }
// })
// });
// });
...
//     req.body.CreationDate = CreationDate;  // data Insertion time.
//     req.body.ModificationDate = ModificationDate;   // data Modified time.

//     //end
//     req.body.userId = req.userId;

//     var query = { userId: req.body.userId, chartName: req.body.chartName };
...
//     req.body.chartId = chartId;
//     // console.log(a)
//     var p = config.getDB().then(function (db) {

//         db.collection("Savecharts").findOne(query, function (err, data) {
//             // console.log('data2***', data);
...
//             }
//             else {
//                 db.collection("Savecharts").insertOne(req.body, function (err, obj) {
//                     if (err) res.status(400).json({ success: false, sucerror: err.errors })
//                     else {
//                         res.status(200).json({ success: true, message: 'Chart inserted successfully.', id: req.body.chartId })
//                     }
//                 });
//             }
//         });
...
app.post('/api/chart', function (req, res) {
    ...
    var chartId = randomString(4, '0123456789');
    req.body.metadata.chartId = chartId.toString();
    req.body.options.chartoptionId = chartId.toString();
    // console.log(a)
    var p = config.getDB().then(function (db) {
...
//                 }
//                 else {
//                     db.collection("Savecharts").remove(query, function (err, data) {
//                         if (err) {
//                             res.status(500).json({ success: false, message: 'something went wrong.' });
//                         }
//                         else {
